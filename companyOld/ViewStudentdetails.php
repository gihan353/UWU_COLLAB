            ...
            if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                ...
                if ($degree == '*') {
                    if ($year == "*") {
                        ...
                        $result = mysqli_query($conn, "SELECT * FROM student WHERE ac_year = '" . $year . "'");
                    }
                } else {
                    if ($year == "*") {
                        $result = mysqli_query($conn, "SELECT * FROM student WHERE degree = '" . $degree . "'");
                    } else {
                        $result = mysqli_query($conn, "SELECT * FROM student WHERE degree = '" . $degree . "' AND ac_year = '" . $year . "'");
                    }
                }
                        ...
                        <form method="POST">
                            <table><tr>
                                    ...
                                    <td><select  class="form-control" id="year" name="year">
                                            ...
                                            <option value="2013/2014" <?php if($_POST['year']=='2013/2014'){echo 'selected';} ?>>2013/14</option>
                                            <option value="2014/2015" <?php if($_POST['year']=='2014/2015'){echo 'selected';} ?>>2014/15</option>
                                            <option value="2015/2016" <?php if($_POST['year']=='2015/2016'){echo 'selected';} ?>>2015/16</option>
                                            <option value="2016/2017" <?php if($_POST['year']=='2016/2017'){echo 'selected';} ?>>2016/17</option>
                                        </select></td>
...
<!--                                    <th>St_ID</th>-->
                            ...
                            <tbody>
                                ...
                                while ($row = mysqli_fetch_array($result)) {
                                    ?>
                                    <tr>
                                        <?php echo"<td>$row[st_name]</td><td>$row[gender]</td><td>$row[DOB]</td><td>$row[faculty]</td><td>$row[department]</td><td>$row[degree]</td><td>$row[ac_year]</td><td>$row[address]</td><td><a href ='sendEmail.php'?st_id=$row[st_id]>$row[email]</a></td>
           
           <td>       <a href='SendEmail.php?job_id=" . $row['st_name'] . "'  </td>"; ?>
                                    </tr> 
                                <?php } ?>
                            </tbody>
                        </table>
